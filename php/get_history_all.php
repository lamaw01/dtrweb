...
if($_SERVER['REQUEST_METHOD'] == 'POST' && array_key_exists('id', $input)){
    ...
    WHERE tbl_logs.id < ? AND tbl_logs.time_stamp BETWEEN ? AND ? AND tbl_employee.name IS NOT NULL
    ...
    try {
        ...
        $get_history_all->bindParam(1, $id, PDO::PARAM_STR);
        $get_history_all->bindParam(2, $date_from, PDO::PARAM_STR);
        $get_history_all->bindParam(3, $date_to, PDO::PARAM_STR);
